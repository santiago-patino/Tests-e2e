...
async function takeScreenshotEveryStep(driver, fileNamePasoEscenario) {
  ...
  const dirPath = path.join(__dirname, "screenshots_members_pseudo");
...
async function fetchDataFromAPI() {
  try {
    const response = await axios.get(
      "https://my.api.mockaroo.com/members_api.json?key=8953aa80"
    );
...
// Datos inválidos
async function fetchInvalidDataFromAPI() {
  try {
    const response = await axios.get(
      "https://my.api.mockaroo.com/members_invalidad_data.json?key=8953aa80"
    );
    return response.data;
  } catch (error) {
    console.error("Error al obtener datos del API:", error);
...
When(
  "Ir a crear un nuevo member con datos pseudoaleatorios",
  async function () {
...
When("Ingresar el nombre pseudoaleatorio del member", async function () {
  const randomName = nombre;
  let element = await this.driver.$("#member-name");
  await element.setValue(randomName);
});

When("Ingresar el email pseudoaleatorio del member", async function () {
...
Then("Debería ver el nuevo member pseudoaleatorio creado", async function () {
...
When(
  "Ir a la sección members para editar el member pseudoaleatorio creado",
  async function () {
    await new Promise((resolve) => setTimeout(resolve, 3000));
    await this.driver.url("https://ghost-xefe.onrender.com/ghost/#/members");
...
Then("Verificar que existe el member a editar", async function () {
...
Then("Debería ver el member pseudoaleatorio editado", async function () {
  await new Promise((resolve) => setTimeout(resolve, 3000));
  await this.driver.url("https://ghost-xefe.onrender.com/ghost/#/members");
  let elementMemberstwoChance = await this.driver.$('a[href="#/members/"]');
  await elementMemberstwoChance.click();
...
When(
  "Ir a la sección members para eliminar el member pseudoaleatorio creado",
  async function () {
    await new Promise((resolve) => setTimeout(resolve, 3000));
    await this.driver.url("https://ghost-xefe.onrender.com/ghost/#/members");
    let elementMemberstwoChance = await this.driver.$('a[href="#/members/"]');
...
Then("Verificar que existe el member creado", async function () {
  let memberNameElement = await this.driver.$("h3");
  let memberName = await memberNameElement.getText();
  expect(memberName).to.include(nombre);
  await memberNameElement.click();
...
When("Borrar el member pseudoaleatorio creado", async function () {
...
Then("No debería existir el member pseudoaleatorio", async function () {
...
When(
  "Ir a crear un nuevo member con email pseudoaleatorio inválido",
  async function () {
    await this.driver.url("https://ghost-xefe.onrender.com/ghost/#/members");
    let element = await this.driver.$("a.ember-view.gh-btn.gh-btn-primary");
    // Llamado a la API
    const data = await fetchInvalidDataFromAPI();
    nombre = data.email;
    email = data.email;
    await takeScreenshotEveryStep(
      this.driver,
...
When(
  "Ingresar el email pseudoaleatorio inválido del member",
  async function () {
    let randomEmail = email;
    let element = await this.driver.$("#member-email");
    await element.setValue(randomEmail);
...
Then(
  "Debería recibir un error de email inválido pseudoaleatorio",
  async function () {
...
When(
  "Ir a crear un nuevo member con nombre pseudoaleatorio inválido",
  async function () {
    await this.driver.url("https://ghost-xefe.onrender.com/ghost/#/members");
    let element = await this.driver.$("a.ember-view.gh-btn.gh-btn-primary");
    // Llamado a la API
...
When(
  "Ingresar el nombre pseudoaleatorio inválido del member",
  async function () {
    const randomName = nombre;
    let element = await this.driver.$("#member-name");
    await element.setValue(randomName);
...
When("Ingresar el email pseudoaleatorio válido del member", async function () {
  let randomEmail = email;
  let element = await this.driver.$("#member-email");
  await element.setValue(randomEmail);
  await new Promise((resolve) => setTimeout(resolve, 3000));
...
Then(
  "Debería recibir un error de nombre inválido pseudoaleatorio",
  async function () {
    let responseElement = await this.driver.$(
