...
for (let i = 0; i < 6; i++) {
    ...
    data.push({
        title: jsonPage[randomNumber].title.replace(/'/g, ''),
        description: jsonPage[randomNumber].description
    });
}
...
When('I create a page with title and content a-priori {kraken-string}', async function (scenario) {
...
Then('validate created or edit page with title a-priori {kraken-string}', async function (scenario) {  
...
Given('select page from list with title a-priori {kraken-string}', async function (scenario) {  
...
When('I go to publish page', async function () {
...
Then('validate url page created with title a-priori {kraken-string}', async function (scenario) {
...
When('I go to edit page with a title a-priori {kraken-string}', async function (scenario) {
    let elementTitle = await this.driver.$(".gh-editor-title");
    await elementTitle.setValue(data[parseInt(scenario)-1].title);
...
When('I go to deleted page', async function () {   
    let elementSettings = await this.driver.$('button.settings-menu-toggle.gh-btn.gh-btn-editor.gh-btn-icon.icon-only.gh-btn-action-icon');
    await elementSettings.click();
...
Then('validate deleted page with title a-priori {kraken-string}', async function (scenario) {
     // Espera a que la lista de páginas esté visible
     await this.driver.$(".pages-list").waitForDisplayed();
...
Given('I go to page url with title a-priori {kraken-string}', async function (scenario) {   
    const urlSlug = titleToUrlSlug(data[parseInt(scenario)-1].title);
    const baseUrl = 'https://ghost-xefe.onrender.com/';
    const fullUrl = baseUrl + urlSlug;
...
When('Add nav bar the URLpage with title a-priori {kraken-string}', async function (scenario) {
    const title = data[parseInt(scenario)-1].title
...
Then('validate acces from nav bar a-priori {kraken-string}', async function (scenario) {   
    const urlSlug = titleToUrlSlug(data[parseInt(scenario)-1].title);
    const baseUrl = 'https://ghost-xefe.onrender.com/';
    const fullUrl = baseUrl + urlSlug;
